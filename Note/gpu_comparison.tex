...
\label{sec:discussion_and_observations}
    ...
    \begin{itemize}
        \item \textbf{Generational Architecture Parity (Ada Lovelace - L40S \& RTX 4090):} 
            Both the NVIDIA L40S and RTX 4090 are built on the identical Ada Lovelace architecture, outperforming all previous-generation cards. Their execution times are virtually identical. This generational scaling cuts execution times to nearly one-third of the RTX A6000 baseline.
        ...
        \item \textbf{Workload Characteristics and VRAM Utilization (A100/V100 Saturation):} 
        On the ASGC DiCOS platform, the data-center grade NVIDIA A100 (Ampere) and V100 (Volta) cards yield almost the same training durations. Because our current Dense-NN workload is relatively lightweight, it does not fully utilize these flagship GPUs. Instead, the training performance becomes purely bound by raw computational speed (FLOPS). Consequently, the newer Ada Lovelace cards (L40S and RTX 4090) outperform both the A100 and V100 by nearly a factor of two.
    \end{itemize}
